...
import dk.jot2re.mult.gilboa.ot.otextension.PseudoOtp;
...
public class NaorPinkasOt implements Ot {
  ...
  private Pair<byte[], byte[]> sendRandomOt() {
    ...
    BigInteger publicKeyZero = network.receive(otherId);
  ...
  private byte[] receiveRandomOt(boolean choiceBit) {
    BigInteger c = network.receive(otherId);
    ...
    BigInteger encZero = network.receive(otherId);
    BigInteger encOne = network.receive(otherId);
    byte[] message;
    if (choiceBit == false) {
